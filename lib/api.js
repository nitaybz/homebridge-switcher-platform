...
function switcherCommand(method, deviceIP, deviceID) {
	return new Promise((resolve, reject) => {
		var jsonPyResponse = ""
		if (debug)
			log('Running Command: python ' + path.join(__dirname , 'switcher.py') + ' ' + method + ' ' + deviceIP + ' ' + deviceID )
		var pyResponse = spawn('python', [path.join(__dirname , 'switcher.py'), method, deviceIP, deviceID])
