...
function switcherCommand(method, deviceIP, deviceID) {
	return new Promise((resolve, reject) => {
		var jsonPyResponse = ""
		if (debug)
			log('Running Command: python ' + path.normalize(__dirname , '/switcher.py') + ' ' + method + ' ' + deviceIP + ' ' + deviceID )
		var pyResponse = spawn('python', [path.normalize(__dirname , '/switcher.py'), method, deviceIP, deviceID])
