...
const messageHandler = function (switcher) {
	if (switcher.device_id in this.switcherDevices) {
		...
		if (switcher.state.remote && this.switcherDevices[switcher.device_id].remote && this.switcherDevices[switcher.device_id].remote !== switcher.state.remote) {
			this.log(`Detected new remote for Breeze device (${switcher.device_id}), changing remote (${this.switcherDevices[switcher.device_id].remote} => ${switcher.state.remote})`)
			const switcherDevice = new Switcher(switcher.device_id, switcher.device_ip, this.log.easyDebug, false, switcher.type, switcher.remote)
